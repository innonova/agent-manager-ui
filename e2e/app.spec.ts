...
test('files view: tree, Monaco, refresh after an agent turn', async ({ page }) => {
  await login(page)
  await page.getByTestId('new-project').click()
  ...
  await expect(page.locator('[data-test=tree-dir][data-path="src"]')).toBeVisible()
  await page.locator('[data-test=tree-dir][data-path="src"]').click()
  await page.locator('[data-test=tree-file][data-path="src/index.ts"]').click()
  await expect(page.getByTestId('file-path')).toHaveText('src/index.ts')
  await expect(page.getByTestId('editor').locator('.view-lines')).toContainText('answer = 42')
  await expect(page).toHaveURL(/path=src(%2F|\/)index\.ts/)
