...
<template>
  ...
  >
    ...
    <span
      v-if="item.streaming"
      class="inline-block h-4 w-2 animate-pulse bg-slate-400 align-text-bottom"
    />
  </div>

  <details v-else-if="item.kind === 'thinking'" class="text-xs text-slate-500" data-item="thinking">
    <summary class="cursor-pointer select-none">thinking</summary>
    <div class="mt-1 border-l-2 border-slate-200 pl-2 whitespace-pre-wrap">{{ item.text }}</div>
  </details>

  <div
    v-else-if="item.kind === 'tool_use'"
    class="rounded border border-slate-200 bg-white text-xs"
    data-item="tool_use"
  >
    <button
    ...
    >
      ...
      <span class="truncate text-slate-500">{{ inputSummary }}</span>
    </button>
    <pre
      v-if="open"
      class="overflow-x-auto border-t border-slate-200 px-2 py-1 whitespace-pre-wrap text-slate-700"
  ...
  <details
    ...
    class="ml-4 text-xs"
    :class="item.isError ? 'text-red-800' : 'text-slate-600'"
    data-item="tool_result"
  >
    <summary class="cursor-pointer select-none">
      {{ item.isError ? 'tool error' : 'result' }} · {{ item.output.length }} chars
    </summary>
    <pre class="mt-1 max-h-80 overflow-auto rounded bg-slate-100 p-2 whitespace-pre-wrap">{{
      item.output
    }}</pre>
  </details>
  ...
  <div
    ...
    class="rounded border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-900"
    data-item="error"
  >
    {{ item.message }}
  </div>

  <div v-else-if="item.kind === 'system'" class="text-xs text-slate-400" data-item="system">
    {{ item.text }}
  </div>
  ...
  <div
    ...
    data-item="turn_end"
  >
    <span class="h-px grow bg-slate-200" />
