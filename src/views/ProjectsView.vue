...
/** Features awaiting review per project, an attention signal on the row; refreshed on feature changes. */
const toReview = ref(new Map<string, number>())
async function loadReview(projectId: string) {
  const { features } = await api.features(projectId).catch(() => ({ features: [] }))
  toReview.value = new Map(toReview.value).set(
    projectId,
    features.filter((f) => f.status === 'review').length,
  )
}
const offFeatures = events.on((f) => {
  if (f.type === 'feature.changed') void loadReview(f.projectId)
})
onUnmounted(() => offFeatures?.())

const agentTotal = (c: Record<string, number>) => Object.values(c).reduce((a, b) => a + b, 0)
...
onMounted(async () => {
  await projects.load()
  for (const r of projects.rows) void loadReview(r.project.id)
...
<template>
  <AppShell>
    ...
    <div class="mx-auto max-w-4xl p-6">
      ...
      <!-- what the machine's accounts are up to, as one quiet line; the rest of the machine is on its own page -->
      <div
        v-if="usage.some((h) => h.accounts.length)"
        class="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-500 dark:text-slate-400"
        data-test="usage-line"
      >
        <template v-for="h in usage" :key="h.host">
          <template v-for="a in h.accounts" :key="h.host + a.profile">
            <span class="flex items-center gap-2"
              ><span>{{ hosts.several ? `${h.host} · ${a.profile}` : a.profile }}</span
      ...
      >
        <li v-for="r in projects.rows" :key="r.project.id" data-test="project-row">
          ...
          >
            ...
            <div class="flex shrink-0 items-center gap-2 text-sm" data-test="project-attention">
              <span
                v-if="r.agentCounts['waiting-permission'] + r.agentCounts['waiting-input']"
                class="rounded bg-amber-100 px-1.5 py-0.5 text-amber-900 dark:bg-amber-900 dark:text-amber-100"
                >{{ r.agentCounts['waiting-permission'] + r.agentCounts['waiting-input'] }} waiting
                for you</span
              >
              <span
                v-if="toReview.get(r.project.id)"
                class="rounded bg-violet-100 px-1.5 py-0.5 text-violet-900 dark:bg-violet-900 dark:text-violet-100"
                >{{ toReview.get(r.project.id) }} to review</span
              >
              <span
                v-if="r.agentCounts.working"
                class="rounded bg-blue-100 px-1.5 py-0.5 text-blue-900 dark:bg-blue-900 dark:text-blue-100"
                >{{ r.agentCounts.working }} working</span
              >
              <span
                v-if="r.agentCounts.error"
